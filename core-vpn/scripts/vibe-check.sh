#!/bin/bash
# Ghost-Scan : WG-FUX Stability Diagnostic
# Version 1.0 ( )
...
# Point to SQLite DB
DB_PATH="${WG_DB_PATH:-/app/data/wg-fux.db}"

get_client_dir() {
  echo "/etc/wireguard/clients/$1/$2"
}

# Ensure SQLite is available
if ! command -v sqlite3 &> /dev/null; then
 log_error "sqlite3 not found. Cannot run diagnostic."
 exit 1
fi
...
if [ -d "$CLIENTS_ROOT" ]; then
 ...
 exists=$(sqlite3 "$DB_PATH" "SELECT count(*) FROM clients WHERE container='$(sql_escape "$container")' AND name='$(sql_escape "$name")';")
 if [ "$exists" -eq 0 ]; then
...
load_config
WG_PEERS=$(wg show "${WG_INTERFACE:-wg0}" peers)
