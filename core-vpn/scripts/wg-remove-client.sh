...
[ $# -ge 2 ] || { log_error "Usage: $0 <container> <client>"; exit 1; }
CONTAINER="$1"
NAME="$2"
...
if [ -f "$CLIENT_DIR/public.key" ]; then
 ...
 if [ -n "$PUBKEY" ]; then
  log_info "Removing peer $PUBKEY from ${WG_INTERFACE:-wg0}..."
  # We don't want this to fail the whole script if the peer is already gone from wg interface
  wg set "${WG_INTERFACE:-wg0}" peer "$PUBKEY" remove 2>/dev/null || log_warn "Peer $PUBKEY not found in ${WG_INTERFACE:-wg0} (skipping interface removal)"
 fi
fi
