...
const { runSystemCommand, writeFileAsRoot, unlinkAsRoot, readdirAsRoot } = require('../services/shell');
const { getWireGuardStats, getClientDir, isValidName, parseWireGuardDump, waitForFile } = require('../services/system');
...
router.post('/', auth, requireManager, async (req, res) => {
  try {
    ...
    log.error('clients', `Client creation failed for ${req.body.name}`, { error: e.message, code: e.code, stack: e.stack });
...
router.post('/:container/:name/toggle', auth, requireManager, async (req, res) => {
  ...
  try {
    ...
    if (enabled) {
      ...
      if (!disabledSuccess) console.error('[API] Warning: Failed to write disabled flag:', disabledError);
      await runSystemCommand(getScriptPath('wg-toggle.sh'), [process.env.WG_INTERFACE || 'wg0', 'peer', publicKey, 'remove']);
    }
  ...
  try {
    ...
    if (uploadLimit !== undefined) {
      ...
      if (updateData.uploadLimit > 0) {
        const { success } = await writeFileAsRoot(path.join(clientDir, 'upload_limit'), String(uploadLimit));
      ...
      const { success: qosSuccess, error: qosError } = await runSystemCommand(getScriptPath('wg-apply-qos.sh'), []);
    ...
    const { success: enfSuccess, error: enfError } = await runSystemCommand(getScriptPath('wg-enforcer.sh'), []);
...
router.post('/bulk-update', auth, requireManager, async (req, res) => {
  ...
  for (const client of clients) {
    try {
      ...
      if (update.quota !== undefined) {
        if (update.quota > 0) await writeFileAsRoot(path.join(clientDir, 'quota'), String(update.quota));
  ...
  const { success: blkSuccess, error: blkError } = await runSystemCommand(getScriptPath('wg-enforcer.sh'), []);
