...
router.post(
  ...
  asyncWrap(async (req, res) => {
    ...
    if (!success) {
      throw createError(error, 'Optimization failed', 'SYSTEM_ERROR');
    }
...
router.get(
  ...
  asyncWrap(async (req, res) => {
    ...
    if (level) {
      // Note: This logic assumes 'auth' type logs have levels,
      // but in current schema 'auth' logs are separate.
      // If we want to filter generic logs by level, we'd change the 'type' filter.
    }
