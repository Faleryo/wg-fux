    ...
    <div className="group space-y-3">
      <div className="flex items-center justify-between px-1">
        ...
        {tooltip && (
          <div className="group/tip relative">
            ...
            <div className="absolute bottom-full right-0 mb-4 w-64 p-4 bg-slate-950/95 backdrop-blur-3xl border border-white/10 rounded-2xl text-[10px] text-slate-400 font-bold uppercase tracking-widest opacity-0 group-hover/tip:opacity-100 transition-all pointer-events-none z-50 shadow-2xl">
              {tooltip}
            </div>
          </div>
        )}
      ...
      <div className="relative group/field">
        ...
        <input
          ...
          value={value}
          onChange={onChange}
          className="relative w-full bg-white/5 border border-white/5 rounded-2xl px-6 py-4 text-white font-mono text-sm focus:outline-none focus:border-white/10 focus:bg-white/10 transition-all"
        />
        {badge && (
          <div className="absolute right-4 top-1/2 -translate-y-1/2 px-3 py-1 bg-slate-900 border border-white/5 rounded-xl text-[9px] font-black font-mono text-slate-500 uppercase tracking-widest">
            {badge}
          </div>
        )}
      </div>
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center bg-slate-900/40 backdrop-blur-3xl p-8 rounded-[3rem] border border-white/5 shadow-2xl gap-8">
        <div className="flex items-center gap-6">
           ...
           <div>
             <h2 className="text-4xl font-black text-white tracking-tighter italic uppercase">Paramètres Noyau</h2>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
         ...
         {/* Content Area */}
         <div className="xl:col-span-3 bg-slate-900/40 backdrop-blur-3xl rounded-[3rem] border border-white/10 p-10 shadow-2xl relative overflow-hidden h-fit">
            {/* Background Icon Watermark */}
            <div className="absolute -top-12 -right-12 p-12 opacity-[0.02] text-white rotate-12 pointer-events-none">
               <Settings size={300} />
            </div>
            ...
            <AnimatePresence mode="wait">
               {activeTab === 'general' && (
                 <motion.div key="gen" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="space-y-8">
                       <h3 className="text-xl font-black text-white flex items-center gap-3 italic uppercase"><Server size={20} className={cn(`text-${theme}-400`)} /> Main-Core</h3>
                    ...
                    <div className="space-y-8">
                       <h3 className="text-xl font-black text-white flex items-center gap-3 italic uppercase"><Zap size={20} className="text-emerald-400" /> Pulse-Mode</h3>
                       <div className="p-6 bg-white/5 rounded-[2rem] border border-white/5 space-y-4">
                          <label className="flex items-center justify-between cursor-pointer group">
                             <div className="text-[10px] font-black text-slate-400 group-hover:text-white uppercase tracking-widest uppercase tracking-widest transition-colors">Persistent Keepalive (25s)</div>
                             <div onClick={() => setConfig({ ...config, keepalive: !config.keepalive })} className={cn(
                               "w-12 h-6 rounded-full transition-all relative border border-white/10 shadow-inner",
                               config.keepalive ? `bg-${theme}-600` : "bg-slate-950"
                             )}>
                                <div className={cn("absolute top-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-white shadow-2xl transition-all", config.keepalive ? "left-[calc(100%-1.25rem)]" : "left-1")} />
                             </div>
                          </label>
                          <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest leading-relaxed">Maintient les sessions actives à travers les pare-feu NAT rigides via stimulation UDP.</p>
                       </div>
                    </div>
                 </motion.div>
               )}
               ...
               {activeTab === 'network' && (
                 <motion.div key="net" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-10">
                    <h3 className="text-xl font-black text-white flex items-center gap-3 italic uppercase"><Globe size={20} className="text-cyan-400" /> Infrastructure</h3>
               ...
               {activeTab === 'security' && (
                 <motion.div key="sec" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-8">
                    <h3 className="text-xl font-black text-white flex items-center gap-3 italic uppercase"><Shield size={20} className="text-rose-400" /> Hardening Profile</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       <div className="p-8 bg-white/5 rounded-[2.5rem] border border-white/5 relative group hover:border-white/10 transition-all">
                          ...
                          <h4 className="text-sm font-black text-white uppercase tracking-widest mb-4 flex items-center gap-3"><ShieldCheck size={18} /> Encryption Module</h4>
                       ...
                       <div className="p-8 bg-white/5 rounded-[2.5rem] border border-white/5 relative group hover:border-white/10 transition-all">
                          <h4 className="text-sm font-black text-white uppercase tracking-widest mb-4 flex items-center gap-3"><Activity size={18} /> Packet Filtering</h4>
               ...
               {activeTab === 'maintenance' && (
                 <motion.div key="maint" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="space-y-8">
                       <h3 className="text-xl font-black text-white flex items-center gap-3 italic uppercase"><Download size={20} className="text-emerald-400" /> Archives</h3>
                    ...
                    <div className="space-y-8">
                       <h3 className="text-xl font-black text-white flex items-center gap-3 italic uppercase"><AlertTriangle size={20} className="text-rose-600" /> Danger Zone</h3>
