...
const EdgeNodeHeatmap = ({ interfaces }) => {
  // Deterministic mock latency/load derived from the interface name so values
  // are stable across re-renders (no Math.random in render → satisfies the
  ...
  const nodes = useMemo(() => {
    ...
    return interfaces.map((iface, i) => {
      const h = hash(iface.name || `iface-${i}`);
      return {
        ...iface,
        latency: 10 + (h % 40),
        load: 10 + ((h >> 6) % 60),
