...
import { Search, X, Package, FileText, Settings, Activity, Users, ShieldCheck, Gauge, Command, ArrowRight } from 'lucide-react';
        ...
        <div className="fixed inset-0 z-[200] flex items-start justify-center pt-16 sm:pt-24 px-4">
          ...
          >
            ...
            <div className="flex items-center gap-4 px-6 py-5 border-b border-white/5 relative z-10">
              <Search size={20} className={cn("flex-shrink-0 transition-colors", query ? `text-${theme}-400` : "text-slate-500")} />
              <input
                ref={inputRef}
                type="text"
                value={query}
            ...
            <div className="max-h-[360px] overflow-y-auto custom-scrollbar p-2">
              {query.length === 0 ? (
                ...
                <div className="py-10 text-center">
                  <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Aucun résultat</p>
                  <p className="text-[10px] text-slate-700 mt-1">pour « {query} »</p>
                </div>
              ) : (
                categories.map(cat => (
                  <div key={cat} className="mb-2">
                    <div className="text-[9px] font-black text-slate-600 uppercase tracking-widest px-3 py-2">{cat}</div>
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            selectedIdx === idx ? `bg-${theme}-600/10 border border-${theme}-500/20` : "hover:bg-white/5 border border-transparent"
                          )}
                        >
            ...
            <div className="px-6 py-3 border-t border-white/5 flex items-center gap-6 text-[9px] font-black text-slate-600 uppercase tracking-widest">
              <span className="flex items-center gap-1.5">
                <kbd className="px-1.5 py-0.5 bg-white/5 rounded border border-white/10">↑↓</kbd> Naviguer
              </span>
              <span className="flex items-center gap-1.5">
                <kbd className="px-1.5 py-0.5 bg-white/5 rounded border border-white/10">↵</kbd> Sélectionner
              </span>
              <span className="flex items-center gap-1.5">
                <kbd className="px-1.5 py-0.5 bg-white/5 rounded border border-white/10">ESC</kbd> Fermer
              </span>
            </div>
          </motion.div>
        </div>
