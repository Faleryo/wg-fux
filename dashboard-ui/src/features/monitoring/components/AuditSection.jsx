...
const AuditSection = () => {
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
      {/* Header */}
      <div className={cn("flex flex-col lg:flex-row justify-between items-center p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border shadow-2xl gap-8 transition-all", isDark ? "bg-slate-900/40 border-white/5 backdrop-blur-3xl" : "bg-white border-black/5")}>
        <div className="flex items-center gap-6">
           <div className={cn("p-5 rounded-[2rem] bg-white/5 shadow-2xl", `text-${theme}-400`)}>
              <ShieldCheck size={36} />
           </div>
           <div>
             <h2 className={cn("text-4xl font-black tracking-tighter italic uppercase transition-colors", isDark ? "text-white" : "text-slate-900")}>Audit de Sécurité</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
         {/* Main Score Board */}
         <div className={cn("xl:col-span-1 rounded-[3rem] border p-10 shadow-2xl relative overflow-hidden flex flex-col items-center justify-center text-center transition-all", isDark ? "bg-slate-900/40 border-white/10 backdrop-blur-3xl" : "bg-white border-black/5 shadow-sm")}>
            <div className="relative w-48 h-48 mb-8 flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                 <span className={cn("text-6xl font-black italic transition-colors", isDark ? "text-white" : "text-slate-900")}>{securityScore}%</span>
                 <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-2">{securityScore > 80 ? 'Safe' : 'Watch'}</span>
              </div>
            </div>
            <h3 className={cn("text-lg font-black uppercase tracking-tighter italic transition-colors", isDark ? "text-white" : "text-slate-900")}>Indice d'Intégrité</h3>
         ...
         <div className="xl:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className={cn("rounded-[3rem] border p-8 shadow-2xl relative group transition-all flex flex-col justify-between", isDark ? "bg-slate-900/40 border-white/5 backdrop-blur-3xl hover:border-white/10" : "bg-white border-black/5 shadow-sm hover:border-indigo-500/20")}>
               <div>
                  <div className="flex items-center justify-between mb-6">
                     <h3 className={cn("text-sm font-black uppercase tracking-widest flex items-center gap-3 italic transition-colors", isDark ? "text-white" : "text-slate-900")}><Shield size={20} /> Firewall Status</h3>
            ...
            <div className={cn("rounded-[2.5rem] border p-8 shadow-2xl relative group transition-all flex flex-col justify-between", isDark ? "bg-slate-900/40 border-white/5 backdrop-blur-3xl hover:border-white/10" : "bg-white border-black/5 shadow-sm hover:border-indigo-500/20")}>
               <div>
                  <div className="flex items-center justify-between mb-6">
                     <h3 className={cn("text-sm font-black uppercase tracking-widest flex items-center gap-3 italic transition-colors", isDark ? "text-white" : "text-slate-900")}><Globe size={20} /> IP Forwarding</h3>
                     {loading ? <RefreshCw className="animate-spin text-slate-700" size={16} /> : (
                       data?.ipForwarding 
            ...
            <div className={cn("rounded-[2.5rem] border p-8 shadow-2xl relative group transition-all flex flex-col justify-between", isDark ? "bg-slate-900/40 border-white/5 backdrop-blur-3xl hover:border-white/10" : "bg-white border-black/5 shadow-sm hover:border-indigo-500/20")}>
               <div>
                  <div className="flex items-center justify-between mb-6">
                     <h3 className={cn("text-sm font-black uppercase tracking-widest flex items-center gap-3 italic transition-colors", isDark ? "text-white" : "text-slate-900")}><Lock size={18} /> Fail2Ban Monitor</h3>
                     {loading ? <RefreshCw className="animate-spin text-slate-700" size={16} /> : (
                       data?.fail2ban 
            ...
            <div className={cn("rounded-[2.5rem] border p-8 shadow-2xl relative group transition-all flex flex-col justify-between", isDark ? "bg-slate-900/40 border-white/5 backdrop-blur-3xl hover:border-white/10" : "bg-white border-black/5 shadow-sm hover:border-indigo-500/20")}>
               <div>
                  <div className="flex items-center justify-between mb-6">
                     <h3 className={cn("text-sm font-black uppercase tracking-widest flex items-center gap-3 italic transition-colors", isDark ? "text-white" : "text-slate-900")}><HardDrive size={18} /> Disk Storage</h3>
                     <span className={cn("text-xl font-mono font-black transition-colors", isDark ? "text-white" : "text-slate-900")}>{data?.disk || '0%'}</span>
                  </div>
