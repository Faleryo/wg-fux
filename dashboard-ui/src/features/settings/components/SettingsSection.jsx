               ...
               {activeTab === 'security' && (
                  <motion.div key="sec" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-8">
                     <div className="flex items-center justify-between">
                        ...
                        <div className={cn("px-4 py-1.5 rounded-full border text-[9px] font-black uppercase tracking-[0.2em]", isDark ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-emerald-50 border-emerald-200 text-emerald-600")}>SENTINEL ACTIVE</div>
                     </div>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className={cn("p-8 rounded-[2.5rem] border relative group hover:border-white/10 transition-all", isDark ? "bg-white/5 border-white/5" : "bg-slate-50 border-black/5 shadow-sm hover:border-indigo-500/20")}>
                           ...
                           <button className="text-[9px] font-black text-indigo-400 hover:text-white uppercase tracking-[0.2em] flex items-center gap-2 transition-all">Analyser Intégrité <ArrowRight size={14} /></button>
                        </div>
                        <div className={cn("p-8 rounded-[2.5rem] border relative group transition-all", isDark ? "bg-white/5 border-white/5 hover:border-white/10" : "bg-slate-50 border-black/5 shadow-sm hover:border-indigo-500/20")}>
                           ...
                           <button className="text-[9px] font-black text-indigo-400 hover:text-white uppercase tracking-[0.2em] flex items-center gap-2 transition-all">Logs Firewall <ArrowRight size={14} /></button>
                        </div>
                     </div>

                     {/* Telegram / Sentinel Test Section */}
                     <div className={cn("grid grid-cols-1 gap-8 mt-4")}>
                        <div className={cn("p-8 rounded-[2.5rem] border relative group transition-all", isDark ? "bg-white/5 border-white/5 hover:border-white/10" : "bg-slate-50 border-black/5 shadow-sm hover:border-indigo-500/20")}>
                             <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                               <div className="space-y-4">
                                  <h4 className={cn("text-sm font-black uppercase tracking-widest flex items-center gap-3 transition-colors", isDark ? "text-white" : "text-slate-900")}><Zap size={18} className="text-amber-400" /> Telegram Sentinel</h4>
                                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-relaxed max-w-xl">Vérifiez la connectivité avec votre bot Telegram. Le système envoie des alertes critiques lors des tentatives de brèche ou de déconnexion d'interface.</p>
                               </div>
                               <button 
                                 onClick={async () => {
                                   try {
                                 ...
                                 className={cn("whitespace-nowrap px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all active:scale-95", isDark ? "bg-white/5 hover:bg-white/10 text-white" : "bg-indigo-600 text-white shadow-lg hover:bg-indigo-500 shadow-indigo-600/20")}
                               >
                                 Tester la Connexion
                               </button>
                             </div>
