...
const STATUS = {
  active: { labelKey: 'invite_active', cls: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' },
  used: { labelKey: 'invite_used', cls: 'text-slate-400 bg-white/5 border-white/10' },
  expired: { labelKey: 'expired_short', cls: 'text-red-400 bg-red-500/10 border-red-500/20' },
};
...
const InvitesPanel = () => {
  ...
  const generate = async () => {
    ...
    try {
      ...
      addToast(t('link_generated_copied'), 'success');
      load();
    } catch (e) {
      addToast(e?.response?.data?.error || t('err_invite'), 'error');
    } finally {
      setBusy(false);
    }
  };
  ...
  const revoke = async (id) => {
    try {
      ...
      addToast(t('invite_revoked'), 'success');
      load();
    } catch (e) {
      addToast(e?.response?.data?.error || t('err_revoke'), 'error');
    }
  };
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="text-[11px] font-black text-slate-500 tracking-[0.25em] border-b border-white/5">
              <th className="px-6 py-3">{t('col_created')}</th>
              <th className="px-6 py-3">{t('col_expires')}</th>
              <th className="px-6 py-3">{t('col_status')}</th>
              <th className="px-6 py-3">{t('col_used_by')}</th>
              <th className="px-6 py-3 text-right">{t('col_action')}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {invites.map((i) => {
              ...
              return (
                <tr key={i.id} className="hover:bg-white/5">
                  ...
                  <td className="px-6 py-3 text-[11px] font-mono text-slate-400">
                    {i.expiresAt ? new Date(i.expiresAt).toLocaleDateString(locale) : '—'}
                  </td>
            ...
            {invites.length === 0 && (
              <tr>
                ...
                >
                  {loading ? t('loading') : t('no_invitation')}
                </td>
              </tr>
            )}
          </tbody>
